...
describe("SYSTEM_PROMPT_TEMPLATE", () => {
  ...
  it("should mention Next Step Coach role", () => {
    expect(SYSTEM_PROMPT_TEMPLATE).toContain("Next Step Coach");
  });

  it("should include coaching-related content", () => {
    // Check for key coaching elements
    expect(SYSTEM_PROMPT_TEMPLATE).toMatch(/follow.?up/i);
  });
});
